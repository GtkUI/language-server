...
export async function activate(context: ExtensionContext) {
  let disposable = commands.registerCommand("helloworld.helloWorld", async uri => {
    ...
    const url = Uri.parse('/home/cole/Documents/Code/gtk-ui/gtk-ui/examples/inheritance.gui');
  ...
  const traceOutputChannel = window.createOutputChannel("GtkUI Language Server trace");
  const command = process.env.SERVER_PATH || "gtkui-language-server";
  ...
  let clientOptions: LanguageClientOptions = {
    ...
    documentSelector: [{ scheme: "file", language: "gui" }],
    synchronize: {
  ...
  client = new LanguageClient("gtkui-language-server", "GtkUI language server", serverOptions, clientOptions);
